...
public class Auth extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try
		{
			...
			stmt= con.prepareStatement("select * from newuser where email=? and password=?");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(rset.next())
		{
			...
			session.setAttribute("FName",strFName);
			String Name=(String) session.getAttribute("FName");
			 response.sendRedirect(request.getContextPath()+"/index.jsp?unm="+strUnm);
			
		}
		else
		{
			response.sendRedirect(request.getContextPath()+"/UserSignup.jsp");
		}
		}catch(SQLException e)
		{
			e.printStackTrace();
